...
import { Plus, Pencil, Trash2, X, Check, Package, Image } from 'lucide-react'
...
export default function ItemManagement() {
  const { auctions, addAuction, updateAuctionStatus } = useAuctions()
  ...
  const handleSave = (data) => {
    if (editItem) {
      // For mock: just show notification (full update needs context method)
      addNotification({ type: 'success', title: 'Item Updated', message: `${data.title} has been updated.` })
    } else {
      addAuction(data)
      addNotification({ type: 'success', title: 'Item Added', message: `${data.title} added successfully.` })
    }
    setEditItem(null)
  }
            ...
            <tbody className="divide-y divide-gray-800/50">
              {filtered.map((a, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-mono text-sm font-bold text-white">{formatCurrency(a.currentBid)}</td>
                  <td className="px-4 py-3 text-right text-sm text-gray-400">{a.totalBids}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={() => {
                          updateAuctionStatus(a.id, 'ended')
                          addNotification({ type: 'warning', title: 'Auction Ended', message: `${a.title} has been force-closed.` })
                        }}
